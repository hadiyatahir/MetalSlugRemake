...
void PlayerSoldier::update(float dt, char** lvl, int cell_size, int mapWidth, int mapHeight, int biomeId)
{
    mMapWidth = mapWidth;
    mCellSize = cell_size;
...
{
    if (mDamageState == DamageState::HEALTHY)
    {
        mDamageState = DamageState::INJURED;
        mDamageTimer = 1.f;
    }
    else if (mDamageState == DamageState::INJURED)
    {
        mDamageState = DamageState::CRITICAL;
        mDamageTimer = 1.f;
    }
    else if (mDamageState == DamageState::CRITICAL)
    {
        mLives--;
        if (mLives <= 0)
            mIsAlive = false;
        ...
        {
            // respawn — reset state, keep inventory
            mDamageState = DamageState::HEALTHY;
            mDamageTimer = 0.f;
            setPosition(600.f, 500.f);  // or your spawn point
        }
    }
}*/
...
/*void PlayerSoldier::handleInput(float dt)
{
    ...
    if (mplayerIndex == 0)
    {
        ...
        fireKey = Keyboard::isKeyPressed(Keyboard::A);   // Z = shoot / attack
...
void PlayerSoldier::takeDamage()
{
    ...
    else if (mDamageState == DamageState::CRITICAL)
    {
        mLives--;
        mDamageTimer = 1.5f;
        mDamageState = DamageState::HEALTHY;  // reset state after losing a life
        if (mLives <= 0)
            mIsAlive = false;
        else
            respawn();
